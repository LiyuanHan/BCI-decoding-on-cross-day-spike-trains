...
for turn in range(turns):
	...
	print('turn: ', turn)
	# today = '2023-11-28-v2-p2-delete-pureMLP-turn_' + str(turn)  # train:pre 4 weeks, test: next week; standard embedding: Day_to + 1
	today = '2023-11-28-v3-p2-delete-pureMLP-turn_' + str(turn)  # train:pre 4 weeks, test: next all; standard embedding: Day_to
		...
		if train_data == 'days':
			if data_from_to == '20221103_20230402_delete':
				day = {'0': 0, '1': 5, '2': 10, '3': 13, '4': 18, '5': 24, '6': 30, '7': 36, '8': 40, '9': 42, '10': 46,
				       ...
				       '20': 71, '21': 74, '22': 81, '23': 86, '24': 88, '25': 90, '26': 92, '27': 94, '28': 96, '29': 98,
				       '30': 100, '31': 101, '32': 103, '33': 106, '34': 110, '35': 114, '36': 118, '37': 120, '38': 122,
	...
	if not os.path.exists(
			filepath + '/deocder_dir_' + decoder_method + '_GY-p2_acrossdays_expect_bin_' + str(
				expect_num_bins) + '_without_align' + '_Day_from_' + str(
				Day_from) + '_to_' + str(
	...
	with torch.no_grad():
		...
		np.save(filepath + '/outputs_data_without_align' + '_GY-p2_acrossdays_Day_from_' + str(
...
np.save(filepath + '/outputs_acc_without_align_data_0' + week_name_from_to + '_GY-p2_acrossdays_Day_from_' + str(
	Day_from) + '_to_' + str(Day_to) + '_testDay_' + str(test_Day) + '_' + 'add_' + str(
	Day_from + add_num) + '_' + str(Day_to + add_num_to) + '_' + 'lr_' + str(
	learning_rate) + '_batch_' + str(batch_size) + '_hidden_' + str(

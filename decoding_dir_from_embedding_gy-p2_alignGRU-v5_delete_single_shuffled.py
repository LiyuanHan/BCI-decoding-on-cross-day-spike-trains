...
for turn in range(turns):
	...
	print('turn: ', turn)
	# today = '2024-01-25-v3-p2-delete_alignGRU_single_turn_' + str(turn)  # train:80%, test: 20%; standard embedding: Day_from (no shuffle)
	# today = '2024-01-25-v4-p2-delete_alignGRU_single_turn_' + str(turn)  # train:80%, test: 20%; standard embedding: Day_from (shuffled), test_align- line 437
	# today = '2024-01-25-v5-p2-delete_alignGRU_single_turn_' + str(turn)  # train:80%, test: 20%; standard embedding: Day_from (shuffled), test_align- line 439
	today = '2024-01-25-vv6-p2-delete_alignGRU_single_turn_' + str(turn)  # train:80%, test: 20%; standard embedding: Day_from + all training, (shuffled), test_align- line 439,
		...
		if not os.path.exists(
			...
			for i in range(day[str(Day_from - 1)], day[str(Day_to)]):
				...
				for h in range(len(id_target_align)):
					# ss += len(id_target_align[h]) # 记录是否取得长度与neural_train一致
	...
	if not os.path.exists(
		...
		for epoch in range(num_epochs):
			...
			correct = 0
		if epoch == num_epochs:
			file_data_not_convergence.append([test_Day, seeds])
	...
	with torch.no_grad():
		...
		np.save(filepath + '/outputs_data_align_method_' + str(
			align_method) + '_0' + week_name_from_to + '_GY-p2_single' + '_testDay_' + str(
			test_Day) + '_' + 'lr_' + str(
			learning_rate) + '_batch_' + str(batch_size) + '_hidden_' + str(hidden_size) + '_method_' + str(
...
np.save(filepath + '/outputs_acc_align_data_align_method_' + str(
	align_method) + '_0' + week_name_from_to + '_GY-p2_single' + '_testDay_' + str(test_Day) + '_' + 'lr_' + str(
	learning_rate) + '_batch_' + str(batch_size) + '_hidden_' + str(
	hidden_size) + '_' + today + '_' + data_from_to + '.npy',
        output_acc_align)
if len(file_data_not_convergence) > 0:
	np.save(filepath + '/file_testday_'+str(test_Day)+'_not_convergence.npy', file_data_not_convergence)
